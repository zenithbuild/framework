// Client-safe search engine used by SearchModal and any server-side consumers.
// All functions are pure string operations and can run in the browser.

import type { SearchRecord, SearchResult, SearchScopeConfig, SearchMatchSnippet } from "@/server/searchTypes";
...
export function highlightToFragments(text: string, tokens: string[]): HighlightFragment[] {
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    if (index > lastIndex) {
      fragments.push({ text: text.slice(lastIndex, index), mark: false });
    }
    fragments.push({ text: match[0], mark: true });
    lastIndex = index + match[0].length;
  }
  if (lastIndex < text.length) {
    fragments.push({ text: text.slice(lastIndex), mark: false });
  }
  if (fragments.length === 0) {
    fragments.push({ text, mark: false });
  }
  return fragments;
}
