...
function isActionHidden(action: Action) {
	if (!action.hideWhenField)
		return false;

	const itemValue = values.value?.[action.hideWhenField];

	return `${itemValue ?? ''}` === `${action.hideWhenValue ?? ''}`;
}
...
const actionList = computed(() => {
	...
	const formattedActions = props.actions
		.filter((action) => !isActionHidden(action))
		.map((action) => {
...
const overflowActions = computed(() => {
	if (!actionList.value || actionList.value.length <= 1)
		return [];
		...
		<div class="header-content" :style="{ '--header-color': color }">
			...
			<div class="actions-wrapper">
				<div class="actions-container">
					...
					<v-menu v-if="hasOverflowActions" placement="bottom-end">
						<template #activator="{ toggle }">
							<div>
								...
								<!-- We show the icon only on smaller containers -->
								<v-button v-tooltip="t('more')" small class="icon-button" icon @click="toggle">
