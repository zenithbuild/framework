...
- Support for a primary action with overflow actions in a dropdown menu
- Dynamic URL templates with field value interpolation
- Optional item-field-based action hiding for ownership-aware workflows
...
Configure buttons that appear in the header. The first visible action is shown as a primary button, and any additional visible actions are placed in an overflow dropdown.
...
- **Hide When Field / Hide When Value** - Optionally hide an action when a root-level item field matches a specific value
