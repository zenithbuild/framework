...
function createFakeNpm(dir) {
  ...
  const script = [
    ...
    '  process.exit(1);',
    '}',
    'function addPackFiles(files, manifest) {',
    '  for (const entry of manifest.files || []) {',
    '    if (entry === "README.md" || entry === "LICENSE" || entry === "package.json") {',
    '      files.add(entry);',
    '      continue;',
    '    }',
    '    if (entry === "bin" || entry === "bin/**") {',
    '      if (manifest.name.includes("bundler")) {',
    '        files.add(manifest.name.includes("win32") ? "bin/zenith-bundler.exe" : "bin/zenith-bundler");',
    '      } else {',
    '        files.add(manifest.name.includes("win32") ? "bin/zenith-compiler.exe" : "bin/zenith-compiler");',
    '      }',
    '    }',
    '    if (entry === "dist" || entry === "dist/**") {',
    ...
    '  addPackFiles(files, manifest);',
    '  process.stdout.write(JSON.stringify([{ files: [...files].map((filePath) => ({ path: filePath })) }]));',
    '  process.exit(0);',
    '}',
    'if (command === "publish") {',
