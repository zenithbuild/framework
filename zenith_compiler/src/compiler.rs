...
use crate::script::{
    extract_script_blocks, ComponentInstanceBinding, ComponentScriptAsset, HoistedOutput,
    HoistedStateBinding,
};
...
pub fn compile_structured_with_source(input: &str, source_path: &str) -> CompilerOutput {
    let (ast, expressions, hoisted, component_scripts, component_instances, markers, events) =
        compile_internal(input, source_path);
...
pub fn compile(input: &str) -> String {
    let (ast, expressions, _hoisted, _component_scripts, _component_instances, _markers, _events) =
        compile_internal(input, "<inline>");
...
fn compile_internal(
    ...
    let (preprocessed, scripts) = extract_script_blocks(input, source_path);
