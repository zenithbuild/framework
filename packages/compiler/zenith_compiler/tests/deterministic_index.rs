...
fn nested_expressions_indexed_depth_first() {
    let input = r#"<div><span>{props.a}</span><span>{props.b}</span></div>"#;
...
fn sibling_expressions_indexed_left_to_right() {
    let input = r#"<div>{props.x}{props.y}{props.z}</div>"#;
    let output = compile(input);

    assert!(output.contains(r#"__zenith_expr = ["props.x", "props.y", "props.z"]"#));
...
fn complex_interleaving_deep_nesting() {
    let input = r#"<div id={props.a}><p class={props.b}>{props.c}<span>{props.d}</span></p><footer>{props.e}</footer></div>"#;
    let output = compile(input);

    // Order: a(attr on div), b(attr on p), c(child of p), d(child of span inside p), e(child of footer)
    assert!(output
        .contains(r#"__zenith_expr = ["props.a", "props.b", "props.c", "props.d", "props.e"]"#));
...
fn multiple_events_indexed_in_source_order() {
    let input = r#"<div on:click={props.handler1} on:hover={props.handler2} />"#;
    let output = compile(input);

    assert!(output.contains(r#"__zenith_expr = ["props.handler1", "props.handler2"]"#));
...
fn index_stability_across_repeated_compilations() {
    let input = r#"<div>{props.a}<span>{props.b}</span>{props.c}</div>"#;
