...
import { createNetlifyBasePathAssetRules, createNetlifyImageEndpointRule, createNetlifyRouteCheckRule, createNetlifyRewriteRules } from './route-rules.js';
...
function buildRedirectsFile(buildManifest: BuildManifest, serverRoutes: HostedServerManifestRoute[]) {
    const lines = [
        ...
        createNetlifyRouteCheckRule(buildManifest.base_path)
    ];
...
export const netlifyAdapter: AdapterDriver = {
    ...
    async adapt(options) {
        ...
        await writeFile(
            join(functionsDir, '__zenith_route_check.mjs'),
            createHostedRouteCheckSource({
                buildManifest: hostedContext.buildManifest,
                pageServerRoutes: serverRoutes.filter(isHostedPageRoute),
                runtimeBasePath: './_zenith',
                routeModuleBaseSegments: ['_zenith', 'routes'],
                exportKind: 'netlify'
            }),
            'utf8'
        );
